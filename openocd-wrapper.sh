...
TOOL_NAME="openocd"
TOOL_MCU_ECLIPSE="`find "${DEVENV_HOME}/tools/toolchain/gnu-mcu-eclipse" -type f -name ${TOOL_NAME}`"

if [ -z "${TOOL_MCU_ECLIPSE}" ]
then
  echo "Don't find ${TOOL_NAME} @ \"${DEVENV_HOME}/tools/toolchain/gnu-mcu-eclipse\""
  exit ${LINENO}
fi
